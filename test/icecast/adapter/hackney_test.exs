defmodule Icecast.Adapter.HackneyTest do
  ...
  describe "adapter options" do
    test "redirects are not followed by default" do
      assert {:error, :redirect} = Hackney.read_meta(@redirect)
    end

    test ":follow_redirect follows the location header" do
      # the redirect target is not a stream, so we get past the redirect and
      # then fail on the missing icy-metaint header
      assert {:error, nil} = Hackney.read_meta(@redirect, follow_redirect: true)
    end

    test ":max_redirects caps the number of hops" do
      assert {:error, {:max_redirects, 1}} =
               Hackney.read_meta(@redirect, follow_redirect: true, max_redirects: 0)
    end
  end
